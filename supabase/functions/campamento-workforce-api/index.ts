...
function validClass(v:any){
  const x=String(v??"").toUpperCase();
  return ["DIRECTA","INDIRECTA","POR_DEFINIR"].includes(x)?x:"";
}
async function saveRules(rules:Record<string,string>){
  const {error}=await db.from("settings").upsert({key:RULE_KEY,value:JSON.stringify(rules)},{onConflict:"key"});
  if(error)throw error;
}
async function audit(category:string,classification:string){
  const {error}=await db.from("audit_log").insert({
    ...
    action:"UPDATE_WORKFORCE_CLASSIFICATION",
    entity_type:"workforce_category",
    entity_id:category,
    endpoint:"campamento-workforce-api",
    result:"OK",
