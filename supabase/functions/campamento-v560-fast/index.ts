...
async function hashState(data:any){const raw=JSON.stringify(data??null);const b=await crypto.subtle.digest('SHA-256',enc.encode(raw));return [...new Uint8Array(b)].map(x=>x.toString(16).padStart(2,'0')).join('').slice(0,32)}
...
async function fallbackState(req:Request,u:URL){
  ...
  if(r.ok&&payload?.ok&&payload?.data){payload.state_version=await hashState(payload.data);payload.server_time=new Date().toISOString();payload.state_engine='raw-fallback'}
  return out(payload,r.status,{'x-camp-state-engine':'raw-fallback'});
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(req.method==='GET'&&action==='advanced_state'){
      ...
      const stateVersion=await hashState(data);
