...
test('PWA modern.12 R5 invalida el shell anterior y mantiene actualización controlada',async({request})=>{
  const version=await request.get('/version.json');expect(version.ok()).toBeTruthy();expect((await version.json()).version).toBe('5.6.1-modern.12-r5-operational-v1');
  const sw=await request.get('/service-worker.js');expect(sw.ok()).toBeTruthy();const text=await sw.text();
  expect(text).toContain("campamento-shell-5.6.1-modern.12-r5-operational-v1");
