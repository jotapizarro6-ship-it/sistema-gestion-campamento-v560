...
async function audit(action:string,result="OK",details:any={},trace?:RequestTrace){const {error}=await db.from("audit_log").insert({profile:"ADMINISTRADOR",action,entity_type:"backup_restore_test",endpoint:"campamento-recovery-api",result,details:traceDetails(details,trace)});if(error)console.error("audit",error)}
...
Deno.serve(async(req:Request)=>{try{
  ...
  if(req.method==="POST"&&action==="restore_test"){
    ...
    if(q.error)throw q.error;
    const result=q.data;
    await requestAudit("PRUEBA_RECUPERACION_RESPALDO",result?.status||"OK",{backup_schema:schema,tables_checked:result?.tables_checked||0,source_rows:result?.source_rows||0,restored_rows:result?.restored_rows||0,duration_ms:result?.duration_ms||0});
