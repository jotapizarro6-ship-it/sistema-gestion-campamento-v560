...
const workflow=read('.github/workflows/validate-modernization.yml');assert.match(workflow,/playwright install --with-deps chromium/);assert.match(workflow,/npm run typecheck/);
const pages=read('.github/workflows/pages.yml');assert.match(pages,/npm run build/);assert.match(pages,/modernization-smoke\.mjs/);
