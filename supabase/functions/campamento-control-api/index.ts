...
async function audit(p:string,action:string,entityType="",entityId="",endpoint="",result="OK",details:any={},trace?:RequestTrace){const {error}=await db.from("audit_log").insert({profile:profile(p),action:clean(action,80),entity_type:clean(entityType,50),entity_id:clean(entityId,80),endpoint:clean(endpoint,100),result:clean(result,20)||"OK",details:traceDetails(safeDetails(details),trace)});if(error)console.error("audit",error)}
...
Deno.serve(async(req:Request)=>{try{
  ...
  const u=new URL(req.url),trace=requestTrace(u),action=u.searchParams.get("action")??"";
  const requestAudit=(
    p:string,
    auditAction:string,
    entityType="",
  ...
  if(action==="action_create"){
    ...
    const q=await db.from("operational_actions").insert(row).select("*").single();if(q.error)throw q.error;await requestAudit(p,"CREAR_ACCION","operational_action",String(q.data.id),action,"OK",{severity,status,category:row.category});return json({ok:true,data:q.data})
  }
  if(action==="action_update"){
    ...
    const q=await db.from("operational_actions").update(patch).eq("id",id).select("*").single();if(q.error)throw q.error;await requestAudit(p,"ACTUALIZAR_ACCION","operational_action",String(id),action,"OK",{status:q.data.status});return json({ok:true,data:q.data})
  }
  if(action==="action_delete"){
    const id=integer(b.id,1,2147483647);if(!id)return json({ok:false,error:"Acción inválida"},400);const q=await db.from("operational_actions").delete().eq("id",id);if(q.error)throw q.error;await requestAudit(p,"ELIMINAR_ACCION","operational_action",String(id),action);return json({ok:true})
  }
  ...
  if(action==="plan_create"){
    ...
    const q=await db.from("master_plan_events").insert(row).select("*").single();if(q.error)throw q.error;await requestAudit(p,"CREAR_HITO","master_plan_event",String(q.data.id),action,"OK",{impact_type:impact,impact_value:row.impact_value,start_date:start});return json({ok:true,data:q.data})
  }
  if(action==="plan_update"){
    ...
    const q=await db.from("master_plan_events").update(patch).eq("id",id).select("*").single();if(q.error)throw q.error;await requestAudit(p,"ACTUALIZAR_HITO","master_plan_event",String(id),action,"OK",{status:q.data.status,impact_type:q.data.impact_type});return json({ok:true,data:q.data})
  }
  if(action==="plan_delete"){
    const id=integer(b.id,1,2147483647);if(!id)return json({ok:false,error:"Hito inválido"},400);const q=await db.from("master_plan_events").delete().eq("id",id);if(q.error)throw q.error;await requestAudit(p,"ELIMINAR_HITO","master_plan_event",String(id),action);return json({ok:true})
  }
  ...
  if(action==="scenario_save"){
    ...
    let q:any;if(b.id){const id=integer(b.id,1,2147483647);q=await db.from("what_if_scenarios").update({name,base_date:base,days,assumptions,summary,created_by_profile:p,updated_at:new Date().toISOString()}).eq("id",id).select("*").single();if(q.error)throw q.error;await requestAudit(p,"ACTUALIZAR_ESCENARIO","what_if_scenario",String(id),action,"OK",{name,days,assumptions:assumptions.length})}else{q=await db.from("what_if_scenarios").insert({name,base_date:base,days,assumptions,summary,created_by_profile:p}).select("*").single();if(q.error)throw q.error;await requestAudit(p,"GUARDAR_ESCENARIO","what_if_scenario",String(q.data.id),action,"OK",{name,days,assumptions:assumptions.length})}return json({ok:true,data:q.data})
  }
  if(action==="scenario_delete"){
    const id=integer(b.id,1,2147483647);if(!id)return json({ok:false,error:"Escenario inválido"},400);const q=await db.from("what_if_scenarios").delete().eq("id",id);if(q.error)throw q.error;await requestAudit(p,"ELIMINAR_ESCENARIO","what_if_scenario",String(id),action);return json({ok:true})
  }

  if(action==="audit"){await requestAudit(p,clean(b.action,80)||"ACCION_FRONTEND",clean(b.entity_type,50),clean(b.entity_id,80),clean(b.endpoint,100),clean(b.result,20)||"OK",b.details);return json({ok:true})}
